...
// Example: protect the vehicle-entries endpoint
app.get('/api/vehicle-entries', requireAdminAuth, async (req, res) => {
  const showAll = req.query.all === '1';
  try {
    const result = await pool.query(
      showAll
...
app.get('/api/vehicle-exit', requireAdminAuth, async (req, res) => {
  const { numberPlate } = req.body;
  try {
    const result = await pool.query(
      `UPDATE vehicle_entries
